...
//
// The reassignment is a single authoritative write-master SQL txn, so live subscribers reconcile:
...
export async function claimDecks(from: string, to: string): Promise<void> {
  const daemon = new HttpRindleDaemonClient({
    ...
    headers: process.env.RINDLE_REPLICATOR_TOKEN
      ? { authorization: `Bearer ${process.env.RINDLE_REPLICATOR_TOKEN}` }
      : undefined,
  })
