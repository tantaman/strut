...
// dev fallback) and return { url } for the component's `src`. Guest-OK auth + a light burst throttle live inside
// uploadArtifactFromRequest. See server/artifact.ts.
export const Route = createFileRoute('/api/artifact')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { uploadArtifactFromRequest } =
          await import('../../server/artifact')
